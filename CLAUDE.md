...
- Makers: 0% fee + 20% rebate of taker fees, **12% initial fill probability** (lottery on submission), 5bps adverse selection. **Post-Only enforced**: maker BUY rejects if `action.price >= bestAsk`; maker SELL rejects if `action.price <= bestBid`. Failed lottery → GTC order.
- **GTC FIFO virtual queue**: GTC orders track `sharesAhead` (book depth at/above limit price on entry). Each tick, depth changes reduce the queue. Order only fills when `sharesAhead <= 0` — modeling real FIFO queue priority. At 5¢ with 10k shares queued, you wait until 10k are consumed. At 50¢ with thin books, fills are fast.
