...
| `calibrateLayer2.py` | Pulls all settled trades from Activity API, buckets by entry price (0.05 increments), computes per-bucket live loss rate, derives the flip probability that would make sim's outcome model match. Writes `config/empirical_flip_prob.json` consumed by `extremeFlipProb()` in `settlement.ts`. Re-run weekly or after a roster swap to keep the curve fresh. | `--write` to save | ✅ active |
| `whaleScan.py` | Weekly snapshot of Polymarket's monthly crypto leaderboard (top 50 by PnL). Saves to `data/whale_scan/<isodate>.json` + diffs vs prior snapshot to surface new entrants, dropouts, and persisters (the wallets with sustained edge worth reverse-engineering). Cron on VPS `0 0 * * 0`. | `--dry-run` | ✅ active (cron weekly Sun 00:00 UTC) |
